...
function setLang(lang) {{
  ...
  if (backBtn) backBtn.textContent = (SEC[lang]||SEC['{default_lang}']).back;
  router();
}}
...
function renderListView() {{
  ...
  document.title = 'JapaneseKitchen';
  renderList();
}}
...
function renderList() {{
  ...
  grid.innerHTML = slice.map(r => {{
    ...
    return `<div class="recipe-thumb" onclick="location.hash='recipe/${{r.id}}'">
      ${{imgHtml}}
...
function renderDetail(id) {{
  ...
  document.title = (r.title[lang]||r.title_ja) + ' | JapaneseKitchen';
  ...
  document.getElementById('d-steps-list').innerHTML = stepsList.map(st => `<li>${{st}}</li>`).join('');
